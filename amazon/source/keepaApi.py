...
class KeepaAPI:
    ...
    def __init__(self, config, BASE_DIR):
        ...
        self.single_asin_cost = 3  # calculate it with all parameters
        self.params = config['params']
        self.IMAGE_DIR = os.path.join(BASE_DIR, 'media/amazon/images')
        self.retry_count = 3  # number of retries in case of an exception

    async def download_image(self, url, asin):
                ...
                async with session.get(self.BASE_URL, params=params) as resp:
                    ...
                    for product in product_data.get('products', []):
                        ...
                        if image_url_str and isinstance(image_url_str, str):
                            ...
                            asyncio.create_task(self.download_image(image_url, product['asin']))

                    return product_data
            except Exception as e:
                logging.error(f'An error occurred while fetching product data: {e}')
            await asyncio.sleep(1)  # wait for 1 second before retry
    ...
    async def bulk_fetch_product_data(self, asin_list_batches, domain_id):
        async with aiohttp.ClientSession() as session:
            for asin_list in asin_list_batches:
                product_data = await self.fetch_product_data(session, asin_list, domain_id)
                if product_data:
                    yield product_data
