...
def ecommerce_app_fetch_model_via_redis():
    LoggingTask.logger.info('ecommerce app fetch model via redis is scheduled...')
    try:
        ...
        if data is not None:
            ...
            LoggingTask.logger.warning('ECOMMERCE:No data found in Redis.')
        
    except redis.RedisError as e:
        LoggingTask.logger.error(f'ECOMMERCE:An error occurred while fetching data from Redis: {e}')
    except json.JSONDecodeError as e:
        LoggingTask.logger.error(f'ECOMMERCE:An error occurred while decoding JSON data from Redis: {e}')

@shared_task(base=LoggingTask)
def ecommerce_app_get_model_via_redis():
    LoggingTask.logger.info('ecommerce app set model via redis is scheduled...')
    try:
...
@shared_task(base=LoggingTask)
def ecommerce_app():    
    LoggingTask.logger.info('ecommerce app is running...')
