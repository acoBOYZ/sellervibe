...
function saveForm() {
    ...
    /** App settings **/
    result = controlElBeforeSave(document, '#app-settings-whiteList', false);
    el = result[0];
    elValue = result[1];
    app['white_list'] = elValue === null ? app['white_list'] : elValue;
...
function loadAllData(data) {
    try {
        ...
        apps.forEach(function(app) {
            checkAndProceedInput(app, 'white_list', document, '#app-settings-whiteList');
...
let webSocket = WebSocketHandler(
    WebSocketAdress('/ws/autoleads-creator/'),
    function(event) {
        let response = JSON.parse(event.data);
        if (response.action === 'get_all') {
            if (response.apps.data.success) {
                loadAllData(response.apps.data);
            } else {
                showToast('error', response.apps.data.error);
            }
        } else if (response.action === 'get_info') {
            if (response.info.success) {
                loadInfoData(response.info.data);
            } else {
                showToast('error', response.info.error);
            }
        } else if (response.action === 'set_all') {
            if (response.response.success) {
                ...
                showToast('error', response.response.error);
            }
        }
    },
    function(event) {
...
function uploadAllData(apps) {
    if (webSocket.getReadyState() === WebSocket.OPEN) {
        ...
        showLottie("upload-to-cloud-error-lottie");
        document.querySelector('.upload-error').classList.remove('hidden');
        showToast('error', 'WebSocket connection is not established');
    }
}