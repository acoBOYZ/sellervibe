      ...
      xhr.onload = function() {
        if (xhr.status == 200) {
          ...
          if (result.success) {
            ...
            showToast('error', result.error.toString());
            last_touch(false);
          }
        }
        else{
          showToast('error', 'In handleFiles went something wrong!');
          last_touch(false);
        }
      }
    }
...
function save_form(){
  ...
  xhr.onload = function() {
    if (xhr.status == 200) {
      ...
      if (response.success) {
        showToast('success', 'Task complate successfully.');
        setTimeout(tab_2_func, 200, name);
        setTimeout(restart_form, 250);
      } else {
        showToast('error', 'Error! Task failed.');
      }
    }
    else{
      showToast('error', 'Error! Task failed for unsported reason! ERR_003');
    }
    setTimeout(function(){
        ...
        xhr.onload = function() {
          if (xhr.status == 200) {
            ...
            if (response.success) {
              ...
              showToast('error', 'Your changes can not saved!');
            }
          }
          else{
            showToast('error', 'Your changes can not saved! Response is not OK!');
          }
        }
...
async function getEmails(name = null) {
  ...
  if(response.ok){
    ...
    showToast('error', 'Failed to fetch emails. Response is not OK!');
    return [];
  }
}
