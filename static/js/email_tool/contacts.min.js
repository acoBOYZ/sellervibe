...
cb_2.addEventListener("click", function(){
  ...
  if(r_2_1.checked){
    ...
    fi_3.addEventListener('change', function(event) {
      event.preventDefault();
        ...
        function(error, data, timeDiff) {
          if (error) {
              ...
              if(data.success){
                ...
                sc_4_1.innerHTML = data.total_count;
                sc_4_2.innerHTML = data.validate_count;
                sc_4_3.innerHTML = data.unknown_count;
                last_touch(true);
...
function getEmailFileNames(pick_list, selected_name='') {
  getAllEmailNames(
    function (data) {
...
function save_form() {
  ...
  if(name == 'Pick one'){
    showToast('error', 'Please select a correct name!');
    return;
  }
  ...
  saveEmails(
    ...
    function() {
      showToast('error', 'Your changes have not been saved!');
      setTimeout(function(){
        cpr_5.style.display = 'none';
        p_5.style.removeProperty('display');
      }, 300);
    ...
    function(progress) {
      pr_5.style.setProperty('--value', `${progress}`);
      pr_5.innerHTML = `${progress}%`;
    },
    false
  );
}
    ...
    function renderTable(emails){
      ...
      deleteButton.addEventListener('click', function() {
        if (document.querySelectorAll('#mtb_1 input[id="emailCheckboxes"]').length === document.querySelectorAll('#mtb_1 input[id="emailCheckboxes"]:checked').length){
          ...
          if(saveChanges()) {
            console.log('remove name from picklist');
          }
        } else {
      ...
      function saveChanges() {
        ...
        let status = true;
        saveEmails(
          ms_1.options[ms_1.selectedIndex].value,
          emails,
          function () {
            showToast('success', 'Your changes have been saved');
          },
