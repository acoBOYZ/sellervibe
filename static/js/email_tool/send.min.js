...
function openSendModal() {
    const modal_send_checkbox = document.getElementById('modal-send-checkbox');
    const modal_send = document.getElementById('modal-send');
    ...
    showLottie('send-emails-lottie');
    modal_send_checkbox.checked = true;
}
...
function updateSendModal(command, color, animate=false){
    ...
    const animate_space = animate ? '<br><br>' : '';
    log_id_context ++;
    const modal_send = document.getElementById('modal-send');
    const modal_log = modal_send.querySelector('#modal-log');
    modal_log.innerHTML += `<pre class="antialiased text-xs/tight text-${color}"><code id="${log_id}">> ${command} </code>${animate_space}</pre>`;
    modal_log.querySelector(`#${log_id}`).scrollIntoView({ behavior: 'smooth' });
    ...
    function animate_func() {
        ...
        modal_log_animate.scrollIntoView({ behavior: 'smooth' });

        if (!stopAnimation) {
            const animationIntervalValue = getRandomDecimal(300, 900);
...
function finishSendModal(){
    ...
    showLottie('send-emails-success-lottie');
}

function addSendModalToButton(innerValue, type){
    const modal_send = document.getElementById('modal-send');
    const modal_action = modal_send.querySelector('.modal-action');
    modal_action.innerHTML += `<label for="modal-send-checkbox" class="btn btn-wide btn-outline btn-${type}">${innerValue}</label>`;
    modal_action.scrollIntoView({ behavior: 'smooth' });

    modal_action.addEventListener('click', function() {
      resetPage();
    });
}
...
function sendFilesToServer() {
    const url = '/tools/upload_attachments/';
    const headers = {};
    const formData = new FormData();

    attachment_list.forEach(attachment => {
      formData.append('attachment', attachment);
      updateSendModal(`found ${attachment.name}`, 'normal');
    });
      ...
      function(error, data, timeDiff) {
        if (error) {
            ...
            if(data.success){
              ...
              if(data.success_responses.length !== 0) {
                attachment_list = data.success_responses.map(item => item.unique_name);
                attachment_list.forEach(fileName => { updateSendModal(`${fileName} uploaded.`, 'success'); });
                data.error_responses.forEach(err => { updateSendModal(`${err.name} failed. ${err.error}`, 'error'); });
                sendEmailsToServer();
              } else {
                updateSendModal('you got some errors while uploading files. Please check your network connection.', 'error');
...
function sendEmailsToServer() {
    const url = '/tools/send_bulk_emails/';
    const headers = {};
    const formData = new FormData();
    ...
    attachment_list.forEach(attachment => {
      formData.append('attachment', attachment);
      updateSendModal(`${attachment} added to email attachments`, 'normal');
    });
    updateSendModal('uploading all attachments', 'accent', true);
  ...
  submit_send.addEventListener('click', function() {
    if(input_from.value.trim() != 'info@sellervibe.co' && !isValidEmail(input_from.value.trim())){
        showToast('error', 'From: segment have not correct email address!<br>Please refresh page and try again.');
        required_area = true;
    }
    ...
    if(input_cc.value.trim()){
        ...
        cc_list.forEach(email => {
            ...
            if(!isValidEmail(trimmedEmail)){
                showToast('warning', `${trimmedEmail} does not valid`);
            } else {
                valid_count++;
    ...
    if(required_area || hasInvalidFields){
        if(required_area){
            showToast('warning', 'Please fill out all required fields correctly.');
        } else {
            showToast('warning', 'There are unsupported fields in your text.<br>Please update the fields and try again.');
            textarea_message.classList.add('border-error', 'border-opacity-75');
        }
        required_area = false;
        input_from_value = '';
